...
const DEFAULT_WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbwK2y5dkBL0y1LNb0v9le8mM0AcXEGsLLvosHh42z8zmQN4ifSSPJmtsR3BrElDY8kr/exec';
...
export async function POST(request: Request) {
  try {
    ...
    const googleSheetWebhook = process.env.GOOGLE_SHEET_WEBHOOK_URL || process.env.GOOGLE_APPS_SCRIPT_URL || DEFAULT_WEBHOOK_URL;
    ...
    if (googleSheetWebhook) {
      try {
        ...
        console.log([GOOGLE SHEETS STATUS]: HTTP  | Response:, resText);
      } catch (sheetError) {
