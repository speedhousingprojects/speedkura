...
export default function InteriorGallerySection() {
  ...
  const handleScroll = () => {
    if (scrollRef.current) {
      ...
      setActiveIndex(Math.min(Math.max(newIndex, 0), INTERIOR_IMAGES.length - 1));
    }
  };
  ...
  return (
    <section id="interiors" className="py-16 sm:py-20 lg:py-28 bg-[#F5F3E6] text-[#1B1717] border-t border-[#3A1C11]/10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 sm:mb-10 gap-4">
          <div>
            <p className="text-xs font-bold text-[#CE793A] uppercase tracking-[0.25em] mb-2 flex items-center gap-2">
              ...
              <span>INTERIORS & LIVING SPACES</span>
            </p>
            <h2 className="font-gumani text-3xl sm:text-4xl lg:text-5xl text-[#3A1C11] leading-tight">
              Spaces designed <span className="italic text-[#CE793A]">for real living.</span>
            </h2>
          </div>

          {/* Slide Counter */}
          <div className="flex items-center">
            <span className="font-gumani text-sm sm:text-base font-bold text-[#3A1C11] bg-[#EEE8DE] px-4 py-1.5 rounded-full border border-[#3A1C11]/10 shadow-sm">
              {String(activeIndex + 1).padStart(2, '0')} / {String(INTERIOR_IMAGES.length).padStart(2, '0')}
            </span>
          </div>
        </div>
        ...
        >
          {INTERIOR_IMAGES.map((img, i) => (
            <div
              ...
              className="w-full sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] flex-shrink-0 snap-center bg-[#EEE8DE] rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg border border-[#3A1C11]/15 group cursor-pointer transition-all hover:-translate-y-1"
            >
